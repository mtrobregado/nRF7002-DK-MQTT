...
#define FAN_LOW "FANLOW"
#define FAN_MED "FANMED"
#define FAN_HIGH "FANHIGH"

// Get the node identifier for gpio1_11
#define GPIO11_NODE	DT_ALIAS(gpio11)
static const struct gpio_dt_spec gpio11 = GPIO_DT_SPEC_GET(GPIO11_NODE, gpios);

#define GPIO12_NODE	DT_ALIAS(gpio12)
static const struct gpio_dt_spec gpio12 = GPIO_DT_SPEC_GET(GPIO12_NODE, gpios);

#define GPIO13_NODE	DT_ALIAS(gpio13)
static const struct gpio_dt_spec gpio13 = GPIO_DT_SPEC_GET(GPIO13_NODE, gpios);
...
static void on_mqtt_publish(struct mqtt_helper_buf topic, struct mqtt_helper_buf payload)
{
	...
	// parse the JSON data 
    cJSON *json = cJSON_Parse(payload.ptr); 
    if (json == NULL) { 
    ...
    if (cJSON_IsString(data) && (data->valuestring != NULL)) { 
		if(strncmp(data->valuestring, FAN_OFF, sizeof(FAN_OFF)-1) == 0){
			gpio_pin_set_dt(&gpio11, 1); // FAN SPEED LOW
			gpio_pin_set_dt(&gpio12, 1); // FAN SPEED MEDIUM
			gpio_pin_set_dt(&gpio13, 1); // FAN SPEED HIGH
		}
		else if(strncmp(data->valuestring, FAN_LOW, sizeof(FAN_LOW)-1) == 0){
			gpio_pin_set_dt(&gpio11, 0); // FAN SPEED LOW
			gpio_pin_set_dt(&gpio12, 1); // FAN SPEED MEDIUM
			gpio_pin_set_dt(&gpio13, 1); // FAN SPEED HIGH
		}
		else if(strncmp(data->valuestring, FAN_MED, sizeof(FAN_MED)-1) == 0){
			gpio_pin_set_dt(&gpio11, 1); // FAN SPEED LOW
			gpio_pin_set_dt(&gpio12, 0); // FAN SPEED MEDIUM
			gpio_pin_set_dt(&gpio13, 1); // FAN SPEED HIGH
		}
		else if(strncmp(data->valuestring, FAN_HIGH, sizeof(FAN_HIGH)-1) == 0){
			gpio_pin_set_dt(&gpio11, 1); // FAN SPEED LOW
			gpio_pin_set_dt(&gpio12, 1); // FAN SPEED MEDIUM
			gpio_pin_set_dt(&gpio13, 0); // FAN SPEED HIGH
		}
		else{
			gpio_pin_set_dt(&gpio11, 1); // FAN SPEED LOW
			gpio_pin_set_dt(&gpio12, 1); // FAN SPEED MEDIUM
...
static void connect_work_fn(struct k_work *work)
{
	...
	struct mqtt_helper_conn_params conn_params = {
		...
		.user_name.ptr = CONFIG_MQTT_SAMPLE_HELPER_USERNAME,
		.user_name.size = strlen(CONFIG_MQTT_SAMPLE_HELPER_USERNAME),
	};
...
static void transport_task(void)
{
	...
	err = gpio_pin_configure_dt(&gpio11, GPIO_OUTPUT_ACTIVE);
	if (err < 0) {
		printk("Cant find custom gpio device gpio11!\n");
		return;
	}

	err = gpio_pin_configure_dt(&gpio12, GPIO_OUTPUT_ACTIVE);
	if (err < 0) {
		printk("Cant find custom gpio device gpio12!\n");
		return;
	}

	err = gpio_pin_configure_dt(&gpio13, GPIO_OUTPUT_ACTIVE);
	if (err < 0) {
		printk("Cant find custom gpio device gpio13!\n");
		return;
	}
